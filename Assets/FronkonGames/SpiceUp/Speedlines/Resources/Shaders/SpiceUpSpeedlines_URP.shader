...
Shader "Hidden/Fronkon Games/Spice Up/Speedlines URP"
{
  ...
  SubShader
  {
    ...
    Pass
    {
      ...
      float Line(const float2 v, const float2 r)
      {
      ...
      float LineNoise(const float2 v)
      {
        float sum = 0.0;
      ...
      half4 SpiceUpFrag(SpiceUpVaryings input) : SV_Target 
      {
        ...
        const float3 tint = lerp(_ColorCenter, _ColorBorder, SafePositivePow_float(length(uv2) * _ColorOffset, _ColorDefinition)) * _ColorBrightness;
