...
namespace FronkonGames.SpiceUp.Speedlines
{
  ...
  public sealed partial class Speedlines
  {
    ...
    private static readonly Speedlines[] NoEffects = new Speedlines[0];
    ...
    public static Speedlines Instance
    {
      get
      {
        ...
        if (pipelineAsset != null)
        {
          ...
          {
            if (scriptableRendererData.rendererFeatures[i] is Speedlines)
              return scriptableRendererData.rendererFeatures[i] as Speedlines;
          }
        }
    ...
    /// <remarks> This function use Reflection, so it can be slow. </remarks>
    private static Speedlines[] Instances
    {
      get
      {
          ...
          for (int i = 0; i < rendererDataList.Length; ++i)
          {
            if (rendererDataList[i] != null && rendererDataList[i].rendererFeatures.Count > 0)
              foreach (var feature in rendererDataList[i].rendererFeatures)
                if (feature is Speedlines)
                  effects.Add(feature as Speedlines);
          }
